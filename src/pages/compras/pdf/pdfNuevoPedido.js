...
export const pdfNuevoPedido = ( items, datos ) => {
    if( items ){
      ...
      doc.text( 'Fecha: ', 4.9, 0.5);
      doc.setFont("QuicksandRegular");
      doc.text( `${ datos.fecha }`, 5.5, 0.5 );
      doc.setFont("QuicksandSemibold");
      doc.text( 'Proveedor: ', 4.9, 0.7);
      doc.setFont("QuicksandRegular");
      doc.text( `${ datos.proveedor }`, 5.7, 0.7 );
      // -------------------- FIN ENCABEZADO --------------------
      ...
      autoTable(doc, {
        ...
        head: [['Nº', 'DESCRIPCION', 'MARCA', 'MODELO', 'CANT']],
        body: items,
      });
      // -------------------- FIN TABLA --------------------         
      doc.save(`PEDIDO Nº ${ datos.fecha }.pdf`);
